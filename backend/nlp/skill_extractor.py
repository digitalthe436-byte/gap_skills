...
from backend.nlp.taxonomy import SYNONYM_MAP, MASTER_TAXONOMY, CATEGORY_META, get_skill_metadata
from backend.parsers.text_cleaner import segment_job_description, clean_text
...
def build_skill_patterns() -> List[Tuple[re.Pattern, str]]:
    ...
    for term in sorted_synonyms:
        ...
        if term in {"c++", "c#", ".net", "ci/cd", "node.js", "vue.js", "next.js"}:
            pattern_str = r"(?<![a-zA-Z0-9])" + escaped + r"(?![a-zA-Z0-9])"
        elif len(term) <= 2:
            # Stricter boundaries for 1-2 letter acronyms (e.g. 'go', 'js', 'ts', 'py', 'r', 'c')
